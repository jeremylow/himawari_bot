...
def set_up_logging(level=logging.DEBUG):
    global logger
    logger = logging.getLogger('HiResLogger')
...
def get_start_coord():
    ...
    logger.debug("Using coordinates: (%d, %d)", lat_px, lng_px)
...
def delete_old_cira_images(num=60):
    ...
    """
    logger.debug("Deleting old CIRA images")
    images = sorted(os.listdir(HIRES_FOLDER))

    num_images_to_del = len(images) - num
    logger.debug("Deleting %d images", num_images_to_del)
...
def make_hires_animation(lat_start=None, lng_start=None):
    ...
    """
    logger.debug("Making hi-res video")
...
def tweet_video(coordinates=None, mp4=None):
    logger.debug("Starting tweet")

    if not mp4:
        refresh_images(num=75)
